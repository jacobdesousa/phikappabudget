...
async function listRevenueCategories(req, res) {
  if (req.query.school_year) {
    ...
    return res.status(200).json(await categoryYears.listForYear("revenue", year));
  }
  const { rows } = await pool.query("SELECT * FROM revenue_categories ORDER BY name ASC");
  res.status(200).json(rows);
}

// Category availability for one year, with what each is carrying that year.
async function listRevenueCategoryYear(req, res) {
  ...
  return res.status(200).json({
    school_year: year,
    categories: await categoryYears.listWithYearState("revenue", year),
    years: await categoryYears.yearsWithCategories("revenue"),
  });
}

async function addRevenueCategoryToYear(req, res) {
  const { id } = idParamSchema.parse(req.params);
  const year = Number(req.params.year);
  if (!Number.isFinite(year)) {
    return res.status(400).json({ error: { message: "Invalid school year" } });
  }
  await categoryYears.addToYear("revenue", id, year);
  return res.status(200).json({ ok: true });
}

// Scoped to one year: this year's entries move to Misc, other years keep the
// category as it was.
async function removeRevenueCategoryFromYear(req, res) {
  const { id } = idParamSchema.parse(req.params);
  const year = Number(req.params.year);
  if (!Number.isFinite(year)) {
    return res.status(400).json({ error: { message: "Invalid school year" } });
  ...
  try {
    const result = await categoryYears.removeFromYear("revenue", id, year, ensureMiscCategoryId);
...
async function importRevenueCategoryYear(req, res) {
  ...
  return res.status(200).json(await categoryYears.importFromYear("revenue", fromYear, toYear));
}
...
async function createRevenueCategory(req, res) {
  ...
  if (Number.isFinite(year)) {
    await categoryYears.addToYear("revenue", result.rows[0].id, year);
  }
