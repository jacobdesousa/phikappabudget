...
export default function ExpensesConfigPage() {
  ...
  async function handleAdd() {
    ...
    const res = await addExpenseCategory({ name: name.trim() });
    if (!res.ok) {
      setError(res.error?.message ?? "Could not add category.");
      return;
    }
    setAddOpen(false);
    setName("");
  ...
  async function handleEditSave() {
    ...
    const res = await updateExpenseCategory(editing.id, { name: name.trim() });
    if (!res.ok) {
    ...
    <Stack spacing={2}>
      <Paper elevation={0} sx={{ p: 2, border: "1px solid", borderColor: "divider" }}>
        <Stack direction={{ xs: "column", sm: "row" }} spacing={2} justifyContent="space-between">
          <Stack spacing={0.5}>
            <Typography variant="h5">Expenses Config</Typography>
            <Typography variant="body2" color="text.secondary">
              Manage expense categories (used when creating expense entries).
            </Typography>
          </Stack>
      ...
      <Dialog open={deleteOpen} onClose={() => setDeleteOpen(false)} fullWidth maxWidth="sm">
        ...
        <DialogContent dividers>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
            Any expense entries still filed under it move to <b>Misc</b> — nothing is deleted,
