...
export default function SetupShiftsPage() {
  const router = useRouter();
  const { can } = useAuth();
  const canWrite = can("shifts.setup.write");
    ...
    <Stack spacing={2}>
      <Paper elevation={0} sx={{ p: 2, border: "1px solid", borderColor: "divider" }}>
        <Stack direction={{ xs: "column", sm: "row" }} spacing={2} justifyContent="space-between" alignItems={{ sm: "center" }}>
          <Box>
            <Typography variant="h5">Setup Shifts</Typography>
            <Typography variant="body2" color="text.secondary">
              Schedule and track chapter setup shifts.
            </Typography>
          </Box>
        ...
        <Box sx={{ display: "flex", gap: 2, alignItems: "flex-start", flexDirection: { xs: "column", md: "row" } }}>
          <Stack spacing={1} sx={{ flex: 1, minWidth: 0 }}>
              {shifts.length === 0 && (
                <Paper elevation={0} sx={{ p: 2, border: "1px solid", borderColor: "divider" }}>
                  <Typography variant="body2" color="text.secondary">
                    No setup shifts for {schoolYear}–{schoolYear + 1}.
                  </Typography>
                </Paper>
              )}
              {shifts.map((s) => (
                ...
                <Table size="small">
                  <TableHead>
                    <TableRow>
                      ...
                      <TableCell align="right">Setups</TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {counts.map((c) => (
      ...
      <Dialog open={deleteOpen} onClose={() => setDeleteOpen(false)} fullWidth maxWidth="sm">
        ...
        <DialogContent dividers>
          ...
          <Typography>
            Delete setup shift on <b>{deleteTarget ? dayjs(deleteTarget.event_date).format("MMM D, YYYY") : "—"}</b>?
